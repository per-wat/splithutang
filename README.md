...
- `SUPABASE_ACCESS_TOKEN`: a Supabase Personal Access Token created under
  **Account → Access Tokens**. Scoped tokens need **Project Settings → Read**
  and **Usage Analytics → Read** for this project. Do not prefix it with
  `NEXT_PUBLIC_`.
...
the route. `SUPABASE_ORGANIZATION_SLUG` is not required. If the platform usage
request fails, the page shows the known Free-plan quotas and links to Supabase's
